...
def genArduinoIno(sos32,FName, FN, Fc_Hz,Fs_Hz):
    print('Generating .INO file....')
    #header
    CodeString = '//ButterWorth Low Pass Filter\n'
    CodeString += '//Filter Name:' + FName + '\n'
    CodeString += '//Filter Order:' + str(FN) + '\n'
    CodeString += '//Filter cutoff frequency (-3dB):' + str(Fc_Hz) + 'Hz\n'
...
if __name__ == "__main__":
    ...
    if(len(Arguments) == 5):
        ...
        Fc_Hz = float(Arguments[3]);
        Fs_Hz = float(Arguments[4]);
        print('Low Pass Butterwoth Arguments:');
        print('Name:', FName);
        ...
        header = ''
        genArduinoIno(sos.astype(np.float32),FName, FN, Fc_Hz,Fs_Hz)

        
    else:
        print('Arguments Error!\n',
              'Example: "python LPButter.py Name n Fc_Hz Fs_Hz"\n',
              'Name - Filter Name',
              'N - Filter Order\n',
              'Fc_Hz - Cutoff Frequency (-3dB)\n',
              'Fs_Hz - Sampling Frequency\n',
              'Example: "python LPButter.py LP 4 50 1000"');
